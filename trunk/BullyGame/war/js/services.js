...
function sendData(DisplayID, DataID) {
	...
	var jsonRequest = new Request.JSON({
	    ...
	    onSuccess: function(reponseJSON){
            ...
            var newHtml = "<p>" + reponseJSON.Data + "</p>";
            console.log("new html = " + newHtml);
            
            //get the div id of a html element, and update its contents
            var MyDiv = document.getElementById(reponseJSON.DivId);
...
function getGameInfo()
{
	...
	var jsonRequest = new Request.JSON({
	    ...
	    onSuccess: function(reponseJSON){
            //update the cookies
            TurnNumber = Cookie.write("turnnum", reponseJSON.TurnNumber);
	    },
	    onFailure: function(){
	        alert("Error communicating with server.");
	    }
    	...
    	var jsonRequest = new Request.JSON({
    	    ...
    	    onSuccess: function(reponseJSON){
                if(!reponseJSON.Status) {
    	            ...
    	            GameID = Cookie.write("gameid", reponseJSON.GameID);
    	            PlayerID = Cookie.write("playerid", reponseJSON.PlayerID);
    	            TurnNumber = Cookie.write("turnnum", reponseJSON.TurnNumber);
    	
    	            //update the number of players
    	            var TotalPlayers = document.getElementById("numplayers");
    	            TotalPlayers.innerHTML = reponseJSON.NumberPlayers;
            	} else {
...
function getCard()
{
	...
	var jsonRequest = new Request.JSON({
	    ...
	    onSuccess: function(reponseJSON){
            //create the new html from the json's data
            var CardURL = reponseJSON.CardURL;
            var DiceRoll1 = reponseJSON.DiceRoll1;
            var DiceRoll2 = reponseJSON.DiceRoll2;
...
function checkState()
{
	...
	var jsonRequest = new Request.JSON({
	    ...
	    onSuccess: function(reponseJSON){
            ...
            TurnNumber.innerHTML = reponseJSON.TurnNumber;
